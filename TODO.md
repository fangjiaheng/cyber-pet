...
- 根因：气泡/菜单打开时 CSS padding 从 4px 变为 148px 立即生效，但窗口 IPC 重定位是异步的，导致 1-3 帧间隙中宠物跳动 144px；同时不同模式使用不同 flex align-items 导致水平跳动
- 修复方案：默认窗口始终保持 360×420（与气泡模式一致），宠物始终在同一 CSS 位置 (130, 232)，所有模式统一 padding 和 flex 对齐，气泡出现/消失不再触发任何窗口大小或位置变化
- 涉及文件：`windowSizes.ts`、`petWindowLayout.ts`、`App.css`、`App.tsx`、`main/index.ts`
...
- [x] 修复宠物在气泡/菜单打开时位移跳动（B1 关闭）
- [x] 修复动画菜单播放被鼠标凝视打断的问题
- [x] 移除动作播放的 bubbleText（签到保留）
- [x] 默认窗口改为 360×420，初始位置改为屏幕右下角
